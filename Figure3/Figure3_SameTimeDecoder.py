...
def decode_predictions(dataframe=pd.DataFrame(), borders_full=[], neuron_type='combined'):
    ...
    if neuron_type == 'combined':
        ...
        labels = ['mse_prev_ipsi_' + neuron_type, 'msacc_basecorrected_prev_ipsi_' + neuron_type,
                  'strength_prev_ipsi_' + neuron_type, \
                  'mse_prev_contra_' + neuron_type, 'msacc_basecorrected_prev_contra_' + neuron_type,
                  'strength_prev_contra_' + neuron_type, \
                  'hemifield_prev_' + neuron_type, \
                  'num_targets', 'random_states_' + neuron_type]
    ...
    if neuron_type == 'combined':
        acc_crosscorr['hemifield_prev_' + neuron_type] = [
            'left' if hemifield_prev[i] == 'left' else 'right' if hemifield_prev[i] == 'right' else 'border' \
    ...
    for cro in range(crossvalidate):
        ...
        for k, (train_idx, test_idx) in enumerate(kf.split(X_shuffle, np.round(np.angle(y), 3).astype(str))):
            for t_id, delta_t_train in enumerate(timing):  # train / test decoder in each time step
                ...
                X_train = np.array([X_shuffle[n][delta_t_train] for n in train_idx])
                if intercept == True:
                    X_train = np.array([np.append(1, X_train[i]) for i in range(len(X_train))])
                ...
                if neuron_type == 'combined':
                    ...
                    mse_prev_contra[cro, k, delta_t_train] = (
                        np.mean((circdist(np.angle(X_test[contra_idx].dot(weights)), \
    ...
    if neuron_type == 'combined':
        ...
        acc_crosscorr['msacc_basecorrected_prev_ipsi_' + neuron_type] = np.mean(
            np.mean((-1) * (mse_prev_ipsi - basepred_prev_ipsi) / \
                    basestdpred_prev_ipsi, axis=1), axis=0)
        acc_crosscorr['msacc_basecorrected_prev_contra_' + neuron_type] = np.mean(
            np.mean((-1) * (mse_prev_contra - basepred_prev_contra) / \
...
for mono in monkeys:  # , 'Pe']:#
    for sess in range(max(df_dat_corr['session'].loc[df_dat_corr['monkey'] == mono]) + 1):
        ...
        df_Sa0_corr['spiking_forAligning'] = [
            np.sum(df_Sa0_corr.loc[n, 'sp_train'].toarray()[:, int(df_Sa0_corr['targ_off'][n] + 100): \
        ...
        df_left['bin_sp'] = [[df['bin_sp'][n][t][left] for t in range(len(df['bin_sp'][n]))] for n in
                             range(len(df['bin_sp']))]
        ...
        with open('SameTimeDecoder_200ms.pickle', 'wb') as handle:
