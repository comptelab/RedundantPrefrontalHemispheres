...
reig_strength = 0.042*namp#0.039 * namp  #0.042 * namp                     # non-specific read-out current # 0.04 * namp
...
def generate_stimuli(min_sep_deg=30, n_stim=4):
    """
    Generate n_stim random angles on [0, 360) such that all pairwise
    distances on the circle are >= min_sep_deg.
    """
    while True:
        # random angles
        angles = np.random.rand(n_stim) * 360

        # compute circular distance matrix
        diffs = np.abs(angles[:, None] - angles[None, :])
        # shortest circular distance
        circ_diffs = np.minimum(diffs, 360 - diffs)

        # ignore diagonal, check minimum
        min_dist = circ_diffs[circ_diffs > 0].min()

        if min_dist >= min_sep_deg:
            return angles
####################################################################################
#                                   START SIMULATIONS                              #
####################################################################################
...
def run_simulation(across_factor):
    ...
    simulate_wm(N_excitatory=N_e, N_inhibitory=N_i,
                stimulus_strength=0.3 * namp, Jpos_excit2excit=2.28,  # 1.73 in original
...
def simulate_wm(
        ...
        sigma_weight_profile=6.1, Jpos_excit2excit=1.64, across_factor=0.03,  # sigma_weight_profile=14.4
        # stimulus1_center_deg=180, stimulus2_center_deg=235,
        # distractor_center_deg = 90,
        stimulus_width_deg=50, stimulus_strength=0.04 * namp,
        t1=1, t2=1,
...
#    syn_inhib2inhib.connect(p=1.0)
    ...
    def stimulate_network(t):
        if t >= t_stimulus1_start and t < t_stimulus1_start + t_stimulus_duration:  # Stimulus1
            ...
            left_stim = np.where(HEMI_1 == 0)[0]
            right_stim = np.where(HEMI_1 == 1)[0]
            # stimulus 1A
            for stimR in right_stim:
                # contralateral -> excit_pop (left) gets fully excited by HEMI==1 (right)
                excit_pop.I_stim[stimuli1_target_idx[stimR]] = stimulus_strength * normgauss(xxx, np.pi / 2)
                # ipsilateral -> excit_pop2 (right) gets fully excited by HEMI==0 (left) but not by HEMI==1 (right)
                excit_pop2.I_stim[stimuli1_target_idx[stimR]] = ipsi_percentage * stimulus_strength * normgauss(xxx, np.pi / 2)
            for stimL in left_stim:
                excit_pop2.I_stim[stimuli1_target_idx[stimL]] = stimulus_strength * normgauss(xxx, np.pi / 2)
                excit_pop.I_stim[stimuli1_target_idx[stimL]] = ipsi_percentage * stimulus_strength * normgauss(xxx, np.pi / 2)
    ...
    load = np.random.choice([2, 4])
    stim1C_load = int((load - 2) >= 1)# whether a third stimulus ibs applied
   ...
   # stimuli1 = [70, 180]
    ...
    # define hemifields
    HEMI_1 = np.array([1 if (stimuli1[i] < 90) | (stimuli1[i] > 270) else 0 for i in range(len(stimuli1))])

    sigma = 0.5
    max_repulsion = 0.75
    # add a manual adaptation of DoG shape (i.e. perturbation from visual areas)
    stim2_location = stim2_location_orig + max_repulsion * dog1(sigma, [np.deg2rad(stimuli1[0]-stim2_location_orig)])[0]

    # compute the simulus index
    stimuli1_center_idx = [int(np.round(N_excitatory / 360. * stimuli1[i])) for i in range(len(stimuli1))]
    stim1_width_idx  = int(np.round(N_excitatory / 360. * stimulus_width_deg / 2))
    stimuli1_target_idx = [[idx % N_excitatory
                       for idx in
                       range(s - stim1_width_idx, s + stim1_width_idx + 1)] for s in stimuli1_center_idx]
    ...
    evaluate_wm(across_factor, spike_monitor_excit, synapse_monitor, depression_monitor_excit, \
                ...
                N_excitatory, stimulus_strength, t1, t2, stimuli1, load,\
                HEMI_1, distr_location, stim2_location_orig,\
...
def evaluate_wm(across_factor, spike_monitor_excit,synapse_monitor_excit, depression_monitor_excit,\
                ...
                N_e, stimulus_strength, trial1, trial2, stimuli1, load,\
                HEMI_1,\
    ...
    for area in areas:
        ...
        for w1 in frames:
            ...
            moving_fr_center[area].append(
                len(np.where(abs(monitor['i'][area][idx] - stimuli1[0] / 360 * N_e) < center_neurons / 2)[0]) / \
    ...
    dict = {'stimuli1': [stimuli1], 'load': load,\
            'HEMI1':[HEMI_1],\
    ...
    df = pd.DataFrame(dict)
    df['stimuli1'] = df['stimuli1'].map(list)
    df['HEMI1'] = df['HEMI1'].map(list)
