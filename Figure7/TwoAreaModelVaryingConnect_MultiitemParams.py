...
filename     = "TwoAreaVaryingUntunedConnect_Stim3_MultiitemParams_try"

start_time = time.time()
numcores   = np.max([int(mp.cpu_count()/3)-2, 1])
if numcores <8:
    numcores=1
...
reig_strength = 0.041 * namp#0.036*namp#0.039 * namp  #0.042 * namp                     # non-specific read-out current # 0.04 * namp
...
def run_simulation(across_factor):
    ...
    sim_time            = t_response2_end#t_response2_end#t_delay2_end#t_stimulus2_end+500*ms
...
#    syn_inhib2inhib.connect(p=1.0)
    ...
    stim1_location      = randint(0,359)#180
...
sims = 20001
possible_factors = np.arange(0., 0.012, 0.006)
